...
regsum=sum(sum(Theta1(:,2:end).*Theta1(:,2:end)))+sum(sum(Theta2(:,2:end)...
    .*Theta2(:,2:end)))+sum(sum(Theta3(:,2:end).*Theta3(:,2:end)));
...
for i=1:m
    derta4=output(:,i)-ytemp(:,i);
    derta3=Theta3'*derta4.*sigmoidGradient(z3(:,i));
    derta3=derta3(2:end);
    derta2=Theta2'*derta3.*sigmoidGradient(z2(:,i));
    derta2=derta2(2:end);
    Theta3_grad=Theta3_grad+derta4*(a2(:,i))';
